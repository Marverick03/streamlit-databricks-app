...
# Databricks notebook source
# MAGIC %md
# MAGIC # Personal Loan Propensity Model Training
# MAGIC 
# MAGIC This notebook trains a logistic regression model to predict personal loan propensity.
# MAGIC 
...
dbutils.widgets.text("model_name", "personal_loan_propensity_model", "Model Name")
dbutils.widgets.text("experiment_path", "/Shared/mlops/personal_loan_propensity", "MLflow Experiment Path")
...
print(f"Training Personal Loan Propensity Model")
...
personal_loan_df = customer360_df.withColumn(
    "personal_loan_v1",
    when(
        (col("age").between(25,55)) &
        (col("current_month_credit") > 706.77) &
        (col("current_month_debit") > 1360.19) &
        (col("fraud_transaction_ratio") == 0),
        1
    ).otherwise(0)
)

print(f"Target distribution:")
personal_loan_df.groupBy("personal_loan_v1").count().show()
...
# Select features and label, handle nulls
training_df = personal_loan_df.select(
    "customer_id",
    *feature_columns,
    "personal_loan_v1"
).fillna({"dependents": 0})
...
with mlflow.start_run(run_name=f"personal_loan_{env}") as run:
    # Train the pipeline
...
loan_scores = (
    ...
    .select(
        "customer_id",
        col("probability_array")[1].alias("personal_loan_score")
    )
)

print(f"Generated {loan_scores.count()} personal loan propensity scores")

# COMMAND ----------

# DBTITLE 1,Save Scores
# Save scores to gold table
output_table = f"{catalog}.{schema}.personal_loan_scores_test"
